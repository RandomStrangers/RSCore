...
void MainScreen_SetActive(void) {
	...
	s->title = "&4Ran&5dom &6Str&0ang&8ers &cCore &7v10.0.7";
...
static void SettingsScreen_DPIScaling(struct LCheckbox* w) {
	...
	Window_ShowDialog("Restart required", "You must restart Random Strangers Core before display scaling takes effect");
#else
...
static struct ThemesScreen {
	LScreen_Layout
	struct LButton btnModern, btnClassic, btnRSC;
...
LAYOUTS the_btnRSC[]  = { { ANCHOR_CENTRE, 0 }, { ANCHOR_CENTRE,  -20 } };
...
static void ThemesScreen_RSC(void* w) {
	ThemesScreen_Set(&Launcher_RSCTheme);
}
...
static void ThemesScreen_Show(struct LScreen* s_) {
	...
	LButton_Init(s, &s->btnRSC,  200, 35, "Core",  the_btnRSC);
	...
	s->btnRSC.OnClick  = ThemesScreen_RSC;
